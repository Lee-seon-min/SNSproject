...
public class Camera2BasicFragment extends Fragment
    ...
    /*private final ImageReader.OnImageAvailableListener mOnImageAvailableListener //원래 찍자마자 저장-> 액티비티로 리스너를 옮기고 아래 setter로 지정
    ...
    public void setmOnImageAvailableListener(ImageReader.OnImageAvailableListener listener){
        this.mOnImageAvailableListener=listener; //이미지 캡처시, 리스너 설정
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (facing != null && facing == facingID) { //전면 카메라라면,
    ...
    /*private static class ImageUpLoader implements Runnable {
        ...
        public void run() {
            ...
            UploadTask uploadTask = profileImagesRef.putBytes(bytes); //업로드작업


            Task<Uri> urlTask = uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() { //작업와 함께,
                @Override
            ...
            }).addOnCompleteListener(new OnCompleteListener<Uri>() { //결과에 따른 이벤트
                @Override
                public void onComplete(@NonNull Task<Uri> task) {
                    if (task.isSuccessful()) { //성공시,
                        Uri downloadUri = task.getResult(); //다운로드 URI를 알려줌
                    } else {
                        // Handle failures
                        // ...
                    }
    ...
    private static class ImageSaver implements Runnable {
        ...
        ImageSaver(Image image, File file) {
            mImage = image;
            mFile = file; //이 파일의 경로를 InitinfoActivity에 보내야 하니 CameraActivity에서 핸들링하도록 함. (저장 코드를 가져간다.)
        }
